  ...
  const resolvedImpurities = useMemo(() => {
    return solvData.impurities.filter(ki => enabledImp[ki.name]).map(ki => {
      ...
      if (annotated.length > 0) {
        return { name: ki.name, refPpm: ki.ppm, fromAnnotation: true,
      ...
      {tab === "input" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: st.card, borderRadius: 12, padding: 16 }}>
            ...
            <p style={{ fontSize: 12, color: st.mt, marginBottom: 8 }}>Paste tab-separated peak table. Flags (Weak, Hidden, C13_Sat, 2ary_Solvent) are parsed and handled.</p>
